// File: api/bot-babiya.js

// Import the necessary libraries
const TelegramBot = require('node-telegram-bot-api');
const axios = require('axios');

// --- Configuration for Babiya ---
...
async function fetchAccountDetails() {
  console.log('BABIYA: Attempting to fetch Facebook account details...');
  if (!facebookAccessToken) {
    console.error('BABIYA: Facebook Access Token is missing!');
  ...
  try {
    ...
    console.log('BABIYA: Successfully fetched details from Facebook.');
    return response.data;
  } catch (error) {
    const errorMessage = error.response
      ? error.response.data.error.message
      : error.message;
    console.error('BABIYA: Facebook API Error:', errorMessage);
    throw new Error(
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (message && message.text) {
      ...
      console.log(`BABIYA: Received message from chat ID ${chatId}: "${text}"`);

      if (text === '/start') {
        console.log('BABIYA: Processing /start command.');
        await bot.sendMessage(
          chatId,
          "Hi Babiya! I'm alive! Send /balance to get your ad account details."
        );
      } else if (text === '/balance') {
        console.log('BABIYA: Processing /balance command.');
        await bot.sendMessage(
          chatId,
...
**Current Balance:** ${formattedBalance} ${accountDetails.currency}
          ...
          console.log('BABIYA: Successfully sent balance details.');
        } catch (error) {
          console.error('BABIYA: Error in /balance handler:', error.message);
          await bot.sendMessage(
            chatId,
        ...
        console.log('BABIYA: Processing default message.');
        await bot.sendMessage(
          chatId,
